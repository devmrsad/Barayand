...
$checkCurrencyValueAvailabilityQuery = "SELECT ".$currency."_amount"." AS amount FROM portfolios WHERE portfolio_id = ?;";
$stmt = $pdo->prepare($checkCurrencyValueAvailabilityQuery);
...
if($row["amount"] < $amount){
    # موجودی ناکافی از ارز
    http_response_code(400);
    echo jn(returnOutput("Insufficient balance"));
    exit;
}

# مقدار جدید موجود از ارز پس از فروش
$newBalance = $row["amount"] - $amount;

# ست کردن موجودی جدید ارز
$sellQuery = "UPDATE portfolios SET ".$currency."_amount"." = ? WHERE portfolio_id = ?;";
$stmt = $pdo->prepare($sellQuery);
$stmt->execute([$newBalance, $portfolioID]);
...
# محاسبه قیمت مقدار ارز فروخته شده
$transactionWorthToman = $currencyPrice * $amount;

# افزودن به موجودی تومان
$addToTomanQuery = "UPDATE portfolios SET toman_amount = toman_amount + ? WHERE portfolio_id = ?;";
$stmt = $pdo->prepare($addToTomanQuery);
$stmt->execute([$transactionWorthToman, $portfolioID]);
...
$stmt->execute([$portfolioID, 'sell', $currency, $amount, $transactionWorthToman]);
